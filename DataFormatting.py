...
zips = "ZipCodeLexington.csv"
zipLexington = pd.read_csv(zips)
data = "craigLexington.csv"
craig = pd.read_csv(data)
craig['zipcode'] = NaN

for i in range(0, len(craig.coord)-1):
    valMin = 10
    for j in range(0, len(zipLexington.latitude)-1):
        temp = findDist(craig.latitude[i], craig.longitude[i], zipLexington.latitude[j], zipLexington.longitude[j])
        if temp < valMin:
            valMin = temp
            craig.zipcode[i] = zipLexington.zipcode[j]
...
craig.to_csv("craig11_15Formatted.csv")
